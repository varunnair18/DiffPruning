...
CUDA_VISIBLE_DEVICES=${GPU} python ${BASE_DIR}/examples/run_glue_fixmask_finetune.py --model_type bert --model_name_or_path bert-large-cased-whole-word-masking --task_name ${TASK} --do_test --data_dir ${LOCAL_DATA_DIR}/${DATA} --output_dir ${LOCAL_CKPT_DIR}/${EXP3} --mask_checkpoint "" --structured ${STRUCTURED}
